...
In trying to make our design more time efficient we found it to be useful to “prime” the linear actuator during the initial turnaround, i.e. let it extend just enough that the bullet is about to enter the flywheels. However, in doing so, we used a time-based method of stopping the linear actuator (the appropriate time increment was found during the experimental testing of the system). In general, it is good software design to eliminate time-based coding; since it is known to be inconsistent. The better practice would be to utilize a limit switch to signal when the linear actuator should end its priming state. 
